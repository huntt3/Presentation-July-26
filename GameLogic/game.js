...
function gameUpdate() {
  ...
  // T = drop test crate
  if (keyWasPressed("KeyT")) new Crate(mousePos);

  // E = drop enemy
  if (keyWasPressed("KeyE")) new Enemy(mousePos);

  // X = make explosion
  if (keyWasPressed("KeyX")) explosion(mousePos);

  // M = move player to mouse
  if (keyWasPressed("KeyM")) player.pos = mousePos;
