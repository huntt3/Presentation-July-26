...
class GameObject extends EngineObject {
  ...
  update() {
    ...
    if (!this.isDead() && this.damageTimer.isSet())
      brightness = 0.5 * percent(this.damageTimer, 0.15, 0);
    this.additiveColor = hsl(0, 0, brightness, 0);

    // kill if below level
    if (!this.isDead() && this.pos.y < -9) this.kill();
  }
...
class Crate extends GameObject {
  constructor(pos) {
    super(pos, vec2(0.999), spriteAtlas.crate, (randInt(4) * PI) / 2);

    this.color = hsl(rand(), 1, 0.8);
    this.health = 5;
...
class Coin extends EngineObject {
  constructor(pos) {
    ...
    this.color = hsl(0.15, 1, 0.5);
  }

  render() {
    // make it appear to spin
    const t = time + this.pos.x / 4 + this.pos.y / 4;
    drawTile(this.pos, vec2(0.1, 0.6), 0, this.color); // edge of coin
    drawTile(
      this.pos,
      vec2(0.5 + 0.5 * Math.sin(t * 2 * PI), 1),
      this.tileInfo,
      this.color
    );
  }
...
class Enemy extends GameObject {
  ...
  update() {
    ...
    if (
      this.groundObject &&
      rand() < 0.01 &&
      this.pos.distance(player.pos) < 20
    )
...
class Grenade extends GameObject {
  constructor(pos) {
    super(pos, vec2(0.2), spriteAtlas.grenade);
...
class Weapon extends EngineObject {
  constructor(pos, parent) {
    super(pos, vec2(0.6), spriteAtlas.gun);
    ...
    this.addChild(
      (this.shellEmitter = new ParticleEmitter(
        ...
        0,
        0,
        0.1, // pos, angle, emitSize, emitTime, emitRate, emitCone
        0, // tileInfo
        rgb(1, 0.8, 0.5),
        rgb(0.9, 0.7, 0.5), // colorStartA, colorStartB
        rgb(1, 0.8, 0.5),
        rgb(0.9, 0.7, 0.5), // colorEndA, colorEndB
        3,
        0.1,
        0.1,
        0.15,
  ...
  update() {
    ...
    if (this.triggerIsDown) {
      ...
      for (
        ...
        this.localAngle = -rand(0.2, 0.25);
        this.recoilTimer.set(0.1);
...
class Bullet extends EngineObject {
  ...
  update() {
    ...
    if (this.range < 0) {
      new ParticleEmitter(
        ...
        50,
        PI,
        spriteAtlas.circle, // pos, emit info, tileInfo
        rgb(1, 1, 0.1),
        rgb(1, 1, 1), // colorStartA, colorStartB
        rgb(1, 1, 0.1, 0),
        rgb(1, 1, 1, 0), // colorEndA, colorEndB
        0.1,
        0.5,
        0.1,
        0.05,
        ...
        0.5,
        0,
        1 // randomness, collide, additive, randomColorLinear
      );
  ...
  kill() {
    ...
    const emitter = new ParticleEmitter(
      ...
      0.1,
      100,
      0.5, // pos, angle, emitSize, emitTime, emitRate, emitCone
      0, // tileInfo
      rgb(1, 1, 0),
      rgb(1, 0, 0), // colorStartA, colorStartB
      rgb(1, 1, 0),
      rgb(1, 0, 0), // colorEndA, colorEndB
      0.2,
      0.2,
      0,
      0.1,
